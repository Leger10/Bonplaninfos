...
exports.handler = async (event) => {
    ...
    try {
        ...
        // 🔥 UTILISER L'URL CORRECTE POUR LE TEST LOCAL
        ...
        console.log('💰 Création paiement ticket MoneyFusion:', {
            ...
            fees: personal_Info[0]?.amountWithFees ? personal_Info[0]?.amountWithFees - personal_Info[0]?.amountFcfa : 0,
        ...
        const paymentData = {
            ...
            return_url: return_url || `${process.env.URL || 'http://localhost:8888'}/profile?tab=tickets&payment=success&order=${personal_Info[0]?.orderId || 'unknown'}`,
            webhook_url: webhook_url || `${process.env.URL || 'http://localhost:8888'}/.netlify/functions/moneyfusion-ticket-webhook`
        };
