...
const EventDetailPage = () => {
  ...
  const fetchEventData = useCallback(async () => {
    ...
    try {
      ...
      if (eventData.event_type === 'raffle') {
        ...
        const { data: prizesData, error: prizesError } = await supabase.from('raffle_prizes').select('*').eq('event_id', id).order('rank');
        if (prizesError) throw prizesError;
        specificEventData = { ...raffleData, prizes: prizesData };
        ...
        const { data: standTypesData, error: standTypesError } = await supabase.from('stand_types').select('*').eq('event_id', id).order('base_price');
        if (standTypesError) throw standTypesError;
        specificEventData = { ...standEventData, stand_types: standTypesData };
        ...
        const { data: ticketTypesData, error: ticketTypesError } = await supabase.from('ticket_types').select('*').eq('event_id', id).order('base_price');
        if (ticketTypesError) throw ticketTypesError;
        specificEventData = { ...ticketingData, ticket_types: ticketTypesData };
  ...
  const renderEventSpecificContent = () => {
    ...
    switch(event.event_type) {
      case 'raffle':
        return <RaffleInterface raffleData={eventData} eventId={event.id} isUnlocked={isUnlocked} isOwner={isOwner} onRefresh={fetchEventData} />;
      case 'stand_rental':
        return <StandRentalInterface event={event} standEventData={eventData} isUnlocked={isUnlocked} isOwner={isOwner} onRefresh={fetchEventData} />;
      case 'ticketing':
        return <TicketingInterface event={event} ticketingData={eventData} ticketTypes={eventData.ticket_types} isUnlocked={isUnlocked} isOwner={isOwner} onRefresh={fetchEventData} />;
      case 'voting':
        return <VotingInterface event={event} isUnlocked={isUnlocked} isOwner={isOwner} onRefresh={fetchEventData} />;
      default:
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative rounded-lg overflow-hidden shadow-lg">
              ...
              <Badge className={`absolute top-4 right-4 ${getCategoryColor(event.category_slug)} text-white border-0`}>{getCategoryLabel(event.category_slug)}</Badge>
