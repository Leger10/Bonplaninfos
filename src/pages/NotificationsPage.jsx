...
const NotificationsPage = () => {
  ...
  const markAllAsRead = async () => {
    try {
      ...
      toast({ title: 'Notifications mises à jour', description: 'Toutes les notifications ont été marquées comme lues.', duration: 5000 });
    } catch(error) {
      toast({ title: 'Erreur', description: 'Impossible de marquer les notifications comme lues.', variant: 'destructive', duration: 5000 });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Notifications - BonPlaninfos</title>
          ...
          <Tabs defaultValue="all" className="w-full">
            ...
            <TabsContent value="unread" className="mt-6 space-y-4">
              ...
              {unreadCount === 0 && (
                <div className="text-center py-16">
                    <Bell className="w-16 h-16 mx-auto text-muted-foreground mb-4"/>
                    <p className="text-muted-foreground">Vous n'avez aucune notification non lue.</p>
                </div>
              )}
            </TabsContent>
          </Tabs>
