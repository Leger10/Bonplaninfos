...
const QUICK_FILTERS = [
    { label: "Tendance (7j)", value: "trending" },
    { label: "Populaires par catégorie", value: "popular_by_category" },
    { label: "Gratuits ce week-end", value: "free_weekend" },
    { label: "Dernière chance", value: "ending_soon", icon: AlertTriangle },
];
...
const EventsPage = () => {
    ...
    const fetchUnlockedEvents = useCallback(async () => {
        ...
        try {
            ...
            const allViewedIds = new Set([...data.map(item => item.event_id), ...viewedData.map(item => item.event_id)]);
            setUnlockedEvents(allViewedIds);

        } catch (err) {
            console.error('Error fetching viewed/unlocked events:', err);
        }
    }, [user]);
    ...
    const handleQuickFilter = (value) => {
        setFilters(prev => ({ ...prev, quickFilter: prev.quickFilter === value ? null : value }));
    };
    ...
    const filteredAndSortedEvents = useMemo(() => {
        ...
        if (filters.quickFilter) {
            // Logic for quick filters can be added here
        }
        ...
        return filtered.sort((a, b) => {
            ...
            const isAViewed = unlockedEvents.has(a.id);
            const isBViewed = unlockedEvents.has(b.id);

            if (isAViewed && !isBViewed) return 1;
            if (!isAViewed && isBViewed) return -1;
    ...
    const handleEventClick = async (event) => {
        ...
        if (!unlockedEvents.has(event.id)) {
            try {
    ...
    const availableCities = useMemo(() => {
        if (filters.selectedCountries.length === 0) {
            return Object.values(CITIES_BY_COUNTRY).flat().slice(0, 100); // Limit initial cities to prevent performance issues
        }
        return filters.selectedCountries.reduce((acc, country) => {
            return acc.concat(CITIES_BY_COUNTRY[country] || []);
        }, []);
    }, [filters.selectedCountries]);

    const eventTypes = [
        { name: "Standard", value: 'standard' },
        { name: "Tombola", value: 'raffle' },
        { name: "Vote", value: 'voting' },
        { name: "Salon", value: 'stand_rental' },
        { name: "Billetterie", value: 'ticketing' },
    ...
    return (
        <div className="min-h-screen bg-background">
            <Helmet>
                <title>Événements - BonPlanInfos</title>
                <meta name="description" content="Découvrez tous les événements, concerts, et sorties." />
            </Helmet>
            <main className="container mx-auto max-w-7xl px-4 py-8">
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
                    <h1 className="text-3xl font-bold mb-2">Tous les Événements</h1>
                    <p className="text-muted-foreground mb-6">Trouvez votre prochaine expérience inoubliable.</p>
                      ...
                      <Sheet>
                          ...
                          <SheetContent side="right" className="w-[300px] sm:w-[400px] flex flex-col">
                              ...
                              <div className="py-4 space-y-6 flex-grow overflow-y-auto pr-4">
                                <div>
                                  <h3 className="text-lg font-semibold mb-3">Types d'événement</h3>
                                  <div className="space-y-2">
                                      {eventTypes.map(type => (
                                          <div key={type.value} className="flex items-center space-x-2">
                                              <Checkbox id={`type-${type.value}`} checked={filters.selectedEventTypes.includes(type.value)} onCheckedChange={() => handleFilterChange('selectedEventTypes', type.value)} />
                                              <Label htmlFor={`type-${type.value}`} className="cursor-pointer">{type.name}</Label>
                                          </div>
                                      ))}
                                  </div>
                                </div>
                                  ...
                                  <div className="space-y-2 max-h-40 overflow-y-auto">
                                      {categories.map(cat => (
                                          <div key={cat.id} className="flex items-center space-x-2">
                                              <Checkbox id={`cat-${cat.id}`} checked={filters.selectedCategories.includes(cat.slug)} onCheckedChange={() => handleFilterChange('selectedCategories', cat.slug)} />
                                  ...
                                  <div className="space-y-2 max-h-40 overflow-y-auto">
                                      {COUNTRIES.map(country => (
                                          <div key={country.code} className="flex items-center space-x-2">
                                              <Checkbox id={`country-${country.code}`} checked={filters.selectedCountries.includes(country.name)} onCheckedChange={() => handleFilterChange('selectedCountries', country.name)} />
                                              <Label htmlFor={`country-${country.code}`} className="cursor-pointer">{country.name}</Label>
                                          </div>
                                      ))}
                                  </div>
                                </div>
                                ...
                                <div>
                                  <h3 className="text-lg font-semibold mb-3">Villes</h3>
                                  <div className="space-y-2 max-h-40 overflow-y-auto">
                                      {availableCities.map(city => (
                                          <div key={city} className="flex items-center space-x-2">
                                              <Checkbox id={`city-${city}`} checked={filters.selectedCities.includes(city)} onCheckedChange={() => handleFilterChange('selectedCities', city)} />
                    ...
                    <div className="flex gap-2 flex-wrap mb-6">
                        {QUICK_FILTERS.map(qf => (
                           ...
                           >
                             {qf.icon && <qf.icon className={`w-4 h-4 mr-2 ${qf.value === 'ending_soon' ? 'text-red-400' : ''}`} />}
                             {qf.label}
                           </Button>
                        ))}
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-16 flex flex-col items-center">
                        ...
                        <p className="text-muted-foreground mb-6">Essayez d'ajuster vos filtres ou votre recherche.</p>
            ...
            <AlertDialog open={confirmation.isOpen} onOpenChange={(isOpen) => !isOpen && setConfirmation({ isOpen: false, event: null, cost: 0, costFcfa: 0, onConfirm: null })}>
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>Débloquer l'événement ?</AlertDialogTitle>
                  <AlertDialogDescription>
