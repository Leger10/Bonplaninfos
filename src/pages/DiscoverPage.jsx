...
const DiscoverPage = () => {
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      ...
      try {
        ...
        if (searchQuery) {
          query = query.or(
            `name.ilike.%${searchQuery}%,city.ilike.%${searchQuery}%,country.ilike.%${searchQuery}%`,
          );
        }
  ...
  const getLocationType = (location) => {
    ...
    const typeMappings = {
      "789b35de-0aed-48a2-99cb-dd6ead850f0b": {
        name: "Visite",
        icon: Camera,
        color: "bg-pink-100 text-pink-800 border-pink-200",
      },
      "e26854f8-e52e-43b4-bea7-1d1f74cdb42c": {
        name: "Hôtel",
        icon: Hotel,
            ...
            {locations.map((location) => {
              ...
              return (
                ...
                >
                  ...
                  {status && (
                    <div
                      className={`absolute top-3 left-3 z-10 px-2 py-1 rounded-full text-xs font-semibold flex items-center gap-1 ${status.color} backdrop-blur-sm`}
                    >
                  ...
                  {locationType && (
                    <div
                      className={`absolute top-3 right-3 z-10 px-2 py-1 rounded-full text-xs font-semibold ${locationType.color} border backdrop-blur-sm flex items-center gap-1`}
                    >
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between mb-4 text-xs text-muted-foreground">
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          {Math.floor(
                            (new Date() - new Date(location.created_at)) /
                              (1000 * 60 * 60 * 24),
                          )}
                          j
                        </span>
                    ...
                    >
                      <ExternalLink className="w-4 h-4 mr-2" />Y Aller
                    </Button>
