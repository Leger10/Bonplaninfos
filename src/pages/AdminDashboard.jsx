import React, { useState, useEffect, useCallback } from 'react';
    ...
    import { Users, Settings, BarChart2, Video, Loader2, Briefcase, Wallet, Megaphone, Calendar, Target, Image as ImageIcon, CreditCard, ShieldCheck, Clock, RefreshCw as RefreshCwIcon, UserCheck, History, BookOpen, DollarSign, MapPin, AreaChart, RotateCcw, ShoppingCart, AlertTriangle } from 'lucide-react';
...
const AdminStats = ({ country }) => {
    ...
    const fetchStats = useCallback(async () => {
        ...
        try {
            ...
            toast({ title: 'Erreur de stats', description: error.message, variant: 'destructive' });
        } finally {
            setLoading(false);
        }
    }, [country]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <Card className="glass-effect shadow-lg">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Chiffre d'Affaires (Pièces Créditées)</CardTitle>
    ...
    const LicenseStatus = ({ user }) => {
        ...
        const fetchPartnerData = useCallback(async () => {
            ...
            if (error) {
                toast({ title: "Erreur de partenaire", variant: 'destructive', description: error.message });
            } else {
                setPartner(data);
            }
            setLoading(false);
        ...
        const handleRenewalRequest = async () => {
            ...
            try {
                // This assumes a 'licence_renewal_requests' table exists
                // with columns admin_id, licence_id
                const { error } = await supabase
                    .from('licence_renewal_requests')
                    .insert({
                        admin_id: user.id,
                        licence_id: partner.license_id, // This needs to be the ID from admin_licences, not partners.license_id
                        status: 'pending',
                        request_date: new Date().toISOString()
                    });
                if (error) throw error;
                toast({ title: "Demande envoyée", description: "Votre demande de renouvellement a été envoyée pour approbation." });
            } catch (error) {
                toast({ title: "Erreur", description: "Impossible d'envoyer la demande. Assurez-vous qu'une licence est bien associée. " + error.message, variant: "destructive" });
            } finally {
        ...
        const statusText = expired ? 'Expirée' : 'Active';
        ...
        return (
            <div className={`p-4 mb-6 rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between text-sm gap-4 ${statusColor}`}>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className="font-bold text-lg">Statut de la licence : <span className={`px-2 py-1 rounded-md`}>{partner.license.name} - {statusText}</span></p>
                        <div className="text-xs mt-2 space-y-1">
                            <p>Activée le : {activationDate.toLocaleDateString('fr-FR')}</p>
                            <p>Expire le : {expirationDate.toLocaleDateString('fr-FR')}</p>
                        </div>
                    </div>
                </div>
                <div className="text-center">
                    {expired ? (
                        <p className="font-bold text-lg">Expirée depuis {Math.abs(days)} jours</p>
                    ) : (
                        <>
                ...
                <AlertDialog>
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            <AlertDialogTitle>Confirmer la demande de renouvellement ?</AlertDialogTitle>
                            <AlertDialogDescription>
                                Une notification sera envoyée aux super administrateurs pour examiner votre demande de renouvellement de licence.
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                            <AlertDialogCancel>Annuler</AlertDialogCancel>
                            <AlertDialogAction onClick={handleRenewalRequest} disabled={renewalLoading}>
                                {renewalLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Confirmer"}
                            </AlertDialogAction>
                        </AlertDialogFooter>
                    </AlertDialogContent>
                </AlertDialog>
...
const AdminStatusBanner = ({ status }) => {
  ...
  switch (status) {
    ...
    case 'pending':
      message = "Votre compte est en attente de vérification. Certaines fonctionnalités sont désactivées.";
      colorClass = "bg-yellow-500/20 text-yellow-300";
      break;
    case 'suspended':
      message = "Votre compte administrateur est suspendu. Veuillez contacter le support.";
      colorClass = "bg-orange-500/20 text-orange-300";
      break;
    case 'expired':
      message = "Votre licence a expiré. Veuillez contacter le support pour la renouveler.";
  ...
  return (
    ...
    const AdminDashboard = () => {
      ...
      }, [userProfile, isAdmin, isSuperAdmin, user, getPromotions, getEvents]);
      ...
      if (!user || (!isSuperAdmin && !isAdmin && !isSecretaryBySuperAdmin)) {
        return (
          <div className="min-h-screen bg-background text-foreground p-4 md:p-8">
            <h1 className="text-2xl text-red-500">Accès non autorisé.</h1>
            <p className="text-muted-foreground">Cette section est réservée aux administrateurs.</p>
          </div>
        );
      }

      const dashboardTitle = isSuperAdmin ? "Tableau de bord Super Administrateur" 
        : isAdmin ? `Tableau de bord Admin - ${userProfile.country}`
        ...
        <div className="min-h-screen bg-background text-foreground p-4 md:p-8 relative">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="flex flex-wrap h-auto items-center justify-center rounded-md bg-muted p-1 text-muted-foreground">
                {isSuperAdmin ? (
                  <>
                    <TabsTrigger value="analytics"><BarChart2 className="w-4 h-4 mr-2" />Analyses</TabsTrigger>
                    <TabsTrigger value="credit_stats"><AreaChart className="w-4 h-4 mr-2" />Stats Crédits</TabsTrigger>
                    <TabsTrigger value="activity_log"><BookOpen className="w-4 h-4 mr-2" />Activités</TabsTrigger>
                    <TabsTrigger value="transactions"><History className="w-4 h-4 mr-2" />Transactions</TabsTrigger>
                    <TabsTrigger value="payments"><DollarSign className="w-4 h-4 mr-2" />Paiements</TabsTrigger>
                    <TabsTrigger value="users"><Users className="w-4 h-4 mr-2" />Utilisateurs</TabsTrigger>
                    <TabsTrigger value="secretaries"><UserCheck className="w-4 h-4 mr-2" />Secrétaires</TabsTrigger>
                    <TabsTrigger value="credits"><CreditCard className="w-4 h-4 mr-2" />Crédits</TabsTrigger>
                    <TabsTrigger value="reversed_credits"><RotateCcw className="w-4 h-4 mr-2" />Crédits Annulés</TabsTrigger>
                    <TabsTrigger value="withdrawals"><Wallet className="w-4 h-4 mr-2" />Retraits</TabsTrigger>
                    <TabsTrigger value="withdrawal_history"><History className="w-4 h-4 mr-2" />Hist. Retraits</TabsTrigger>
                    <TabsTrigger value="events"><Calendar className="w-4 h-4 mr-2" />Événements</TabsTrigger>
                    <TabsTrigger value="locations"><MapPin className="w-4 h-4 mr-2" />Lieux</TabsTrigger>
                    <TabsTrigger value="promotions"><Target className="w-4 h-4 mr-2" />Promotions</TabsTrigger>
                    <TabsTrigger value="partners"><Briefcase className="w-4 h-4 mr-2" />Partenaires</TabsTrigger>
                    <TabsTrigger value="badges"><ShieldCheck className="w-4 h-4 mr-2" />Badges</TabsTrigger>
                    <TabsTrigger value="announcements"><Megaphone className="w-4 h-4 mr-2" />Annonces</TabsTrigger>
                    <TabsTrigger value="popups"><ImageIcon className="w-4 h-4 mr-2" />Popups</TabsTrigger>
                    <TabsTrigger value="videos"><Video className="w-4 h-4 mr-2" />Vidéos</TabsTrigger>
                  ...
                  <>
                    ...
                    <TabsTrigger value="locations"><MapPin className="w-4 h-4 mr-2" />Lieux</TabsTrigger>
                    <TabsTrigger value="promotions"><Target className="w-4 h-4 mr-2" />Promotions</TabsTrigger>
                    <TabsTrigger value="credits_history"><CreditCard className="w-4 h-4 mr-2" />Hist. Crédits</TabsTrigger>
                    <TabsTrigger value="reversed_credits"><RotateCcw className="w-4 h-4 mr-2" />Crédits Annulés</TabsTrigger>
                    <TabsTrigger value="withdrawals"><Wallet className="w-4 h-4 mr-2" />Retraits</TabsTrigger>
                    <TabsTrigger value="users"><Users className="w-4 h-4 mr-2" />Utilisateurs</TabsTrigger>
                    <TabsTrigger value="secretaries"><UserCheck className="w-4 h-4 mr-2" />Secrétaires</TabsTrigger>
                   ...
                   <>
                    <TabsTrigger value="credits"><CreditCard className="w-4 h-4 mr-2" />Créditer un compte</TabsTrigger>
                    <TabsTrigger value="reversed_credits"><RotateCcw className="w-4 h-4 mr-2" />Annuler un crédit</TabsTrigger>
                    <TabsTrigger value="withdrawals"><Wallet className="w-4 h-4 mr-2" />Retraits</TabsTrigger>
                    <TabsTrigger value="withdrawal_history"><History className="w-4 h-4 mr-2" />Hist. Retraits</TabsTrigger>
                   </>
                ) : null}
