...
const BoostPage = () => {
  ...
  if (!selectedContent || !selectedPackInfo) {
    toast({ 
      title: "Erreur", 
      description: "Veuillez sélectionner un événement et un pack de promotion.", 
      variant: "destructive" 
    });
    return;
  }
  ...
  const onSuccess = async () => {
    try {
      ...
      // 3. Créer la promotion avec une approche sécurisée
      const promotionData = {
      ...
      // 4. Mettre à jour l'événement SANS last_promoted_at
      const updateData = {
        is_promoted: true,
      ...
      if (error.message.includes("schema cache") || error.message.includes("column")) {
        toast({ 
          ...
          description: "Une mise à jour de la structure de la base de données est nécessaire. Contactez l'administrateur.", 
          variant: "destructive" 
        });
      } else {
        toast({ 
          title: "Erreur lors du boost", 
          description: error.message || "Une erreur est survenue lors de la promotion de l'événement.", 
          variant: "destructive" 
        });
      }
    } finally {
          ...
          <CardContent className="space-y-4">
            <div className="text-center p-4">
              <p className="text-muted-foreground mb-6">
                Tous vos événements sont déjà en promotion active ou vous n'avez pas encore créé d'événements.
              </p>
            </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-lg text-muted-foreground max-w-2xl">
                Augmentez la visibilité de vos événements et atteignez plus de participants avec nos packs de promotion.
              </p>
            </div>
          </div>
        </motion.div>
                ...
                {selectedPackInfo && (
                  <div className="mt-8 p-6 bg-gradient-to-r from-primary/5 to-primary/10 rounded-xl border border-primary/20">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div>
                        <h3 className="text-xl font-bold mb-2">Pack sélectionné : {selectedPackInfo.name}</h3>
                        <div className="flex flex-wrap gap-4 text-sm">
                          <div className="flex items-center">
                            <Zap className="w-4 h-4 text-primary mr-2" />
                            <span>Durée : <strong>{selectedPackInfo.duration_days} jours</strong></span>
                          </div>
                          <div className="flex items-center">
