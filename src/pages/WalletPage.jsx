...
const WalletPage = () => {
  ...
  const formatCoin = (val) => `${val?.toLocaleString() || 0} π`;
  const formatFull = (val) => `${val?.toLocaleString() || 0} pièces (${((val || 0) * COIN_TO_FCFA_RATE).toLocaleString()} FCFA)`;
        ...
        <Card className="bg-card/80 backdrop-blur-sm">
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-between items-center p-3 bg-background rounded-lg">
              ...
              <span className="font-bold text-lg text-green-400">{formatFull(free_coin_balance)}</span>
            </div>
            <div className="flex justify-between items-center p-3 bg-background rounded-lg">
              <span className="font-medium">{t('wallet_page.paid_coins')}</span>
              <span className="font-bold text-lg text-blue-400">{formatFull(coin_balance)}</span>
            </div>
        ...
        <Card className="bg-card/80 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="p-4 rounded-lg bg-background text-center mb-4">
              ...
              <p className="text-sm text-muted-foreground">Soit {((available_earnings || 0) * COIN_TO_FCFA_RATE).toLocaleString()} FCFA</p>
            </div>
