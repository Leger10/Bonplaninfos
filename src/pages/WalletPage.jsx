    ...
    return (
        <div className="min-h-screen bg-background text-foreground p-4 md:p-8">
            <header className="flex justify-between items-center mb-8">
                <h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">Mon Portefeuille</h1>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-8">
                <StatCard 
                    ...
                    title="Solde Total de Pièces"
                    value={totalCoins.toLocaleString()}
                    color="from-blue-500 to-indigo-600"
                    description={`(${free_coin_balance?.toLocaleString() || 0} Gratuites + ${coin_balance?.toLocaleString() || 0} Achetées)`}
                />
                 <StatCard 
                    icon={<DollarSign className="h-6 w-6 text-white/80" />}
                    title="Gains Disponibles (Pièces)"
                    value={available_earnings?.toLocaleString() || 0}
                    color="from-green-500 to-teal-600"
                    description={`≈ ${earningsInFcfa.toLocaleString()} FCFA`}
                />
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                     <Card className="flex flex-col items-center justify-center h-full bg-gradient-to-br from-amber-400 to-orange-500 text-white">
                        ...
                        <CardContent className="flex flex-col items-center">
                            <p className="text-center mb-4">Rechargez votre solde pour ne rien manquer !</p>
                            <Button onClick={() => navigate('/packs')} className="bg-white text-orange-500 hover:bg-white/90">
                               <ShoppingCart className="mr-2 h-4 w-4" /> Voir les packs
                            </Button>
                        </CardContent>
                    </Card>
                </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <Card className="bg-card/80 backdrop-blur-sm">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2"><Coins className="text-primary" /> Détails du Solde de Pièces</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="flex justify-between items-center p-3 bg-background rounded-lg">
                            <span className="font-medium">Pièces Gratuites</span>
                            <span className="font-bold text-lg text-green-400">{free_coin_balance?.toLocaleString() || 0} π</span>
                        </div>
                        <div className="flex justify-between items-center p-3 bg-background rounded-lg">
                            <span className="font-medium">Pièces Achetées</span>
                ...
                <Card className="bg-card/80 backdrop-blur-sm">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2"><Landmark className="text-primary" /> Retrait des Gains</CardTitle>
                        <CardDescription>Convertissez vos gains en argent réel.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="p-4 rounded-lg bg-background text-center mb-4">
                            <p className="text-sm text-muted-foreground">Gains disponibles</p>
                            <p className="text-3xl font-bold text-green-400">{available_earnings?.toLocaleString() || 0} π</p>
                            <p className="text-sm text-muted-foreground">≈ {earningsInFcfa.toLocaleString()} FCFA</p>
                        </div>
                         <Button className="w-full" disabled={(available_earnings || 0) < (adminConfig?.min_withdrawal_pi || 5000)}>
                            Demander un retrait
                        </Button>
                        <p className="text-xs text-muted-foreground text-center mt-2">
                            Minimum pour un retrait : {adminConfig?.min_withdrawal_pi || 5000} pièces.
                        </p>
                    </CardContent>
                </Card>
            </div>
