...
export const AuthProvider = ({ children }) => {
    ...
    if (savedUser) {
      try {
        ...
        if (
          ...
          console.warn(
            `Security Alert: Non-admin email ${parsedUser.email} has admin user_type. Downgrading.`
          );
  ...
  const login = async (email, password) => {
    try {
      const users = JSON.parse(
        localStorage.getItem("bonplaninfos_users") || "[]"
      );
      ...
      if (
        ...
        !ADMIN_EMAILS.includes(foundUser.email)
      ) {
        console.warn(
          `Security Alert: Non-admin email ${foundUser.email} has admin user_type. Downgrading on login.`
        );
  ...
  const register = async (userData) => {
    try {
      ...
      const referralCode = `BP${Math.random()
        .toString(36)
        .substr(2, 8)
        .toUpperCase()}`;
  ...
  const updateUser = (updatedData) => {
    if (
      updatedData.user_type === "admin" &&
      !ADMIN_EMAILS.includes(user.email)
    ) {
      toast({
        title: "Action non autorisée",
        description:
          "Vous ne pouvez pas vous assigner le rôle d'administrateur.",
        variant: "destructive",
      });
      return;
    }
    ...
    const updatedUsers = users.map((u) => {
      if (u.id === updatedUser.id) {
        if (
          ...
          console.warn(
            `Security Alert: Attempt to grant admin user_type to non-admin email ${u.email}. Reverting.`
          );
