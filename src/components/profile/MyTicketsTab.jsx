...
const MyTicketsTab = () => {
  ...
  const fetchTickets = async () => {
    ...
    try {
      ...
      // Requête simplifiée - la vue contient déjà les relations JSON
            ...
            >
              <div className="flex flex-col md:flex-row">
                <div className="bg-muted/30 md:w-32 p-4 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-border">
                  <span className="text-2xl font-bold text-primary">
                    {eventData.event_end_at
                      ? new Date(eventData.event_end_at).getDate()
                      : "--"}
                  </span>
                  <span className="text-xs uppercase font-bold text-muted-foreground">
                    {eventData.event_end_at
                      ? new Date(eventData.event_end_at).toLocaleDateString("fr-FR", { month: "short" })
                      : "--"}
                  </span>
                  <span className="text-xs text-muted-foreground mt-1">
                    {eventData.event_end_at
                      ? new Date(eventData.event_end_at).getFullYear()
                      : ""}
                  </span>
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center text-sm text-muted-foreground mb-3">
                      ...
                      <span className="line-clamp-1">
                        {eventData.location || eventData.city || "Lieu non spécifié"}
                      </span>
                    </div>
                ...
                <div className="p-4 bg-muted/10 flex flex-row md:flex-col items-center justify-center gap-2 border-t md:border-t-0 md:border-l border-border">
                  ...
                  <Button size="sm" className="flex-1 w-full" onClick={() => handleDownload(ticket)} disabled={downloadingId === ticket.id}>
      ...
      <Dialog open={showQrModal} onOpenChange={setShowQrModal}>
        <DialogContent className="sm:max-w-sm text-center">
          <DialogHeader>
            ...
            <DialogDescription>Présentez ce QR Code à l'entrée</DialogDescription>
          </DialogHeader>
          <div className="flex flex-col items-center justify-center py-6">
            <div className="bg-white p-4 rounded-xl border shadow-sm">
              {selectedTicket && (
                <QRCode value={selectedTicket.ticket_code_short || selectedTicket.ticket_number} size={200} level={"H"} />
              )}
            </div>
