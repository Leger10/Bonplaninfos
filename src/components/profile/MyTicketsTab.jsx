  ...
  const fetchTickets = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        *,
        events (
            id, title, event_start_at, event_end_at, location, city, cover_image
        ),
        ticket_types (
    ...
    <div className="space-y-4 animate-in fade-in">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold flex items-center gap-2">
          <Ticket className="text-primary w-5 h-5" /> Mes Billets (
          {tickets.length})
        </h2>
      ...
      <div className="grid gap-4">
        {tickets.map((ticket) => (
          <Card
            ...
            style={{
              borderLeftColor: getComputedColor(ticket.ticket_types?.color),
            }}
          >
              ...
              <div className="bg-muted/30 md:w-32 p-4 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-border">
                <span className="text-2xl font-bold text-primary">
                  {ticket.events?.event_end_at
                    ? new Date(ticket.events.event_end_at).getDate()
                    : "--"}
                </span>
                <span className="text-xs uppercase font-bold text-muted-foreground">
                  {ticket.events?.event_end_at
                    ? new Date(ticket.events.event_end_at).toLocaleDateString(
                        "fr-FR",
                        { month: "short" },
                      )
                    : "--"}
                ...
                <span className="text-xs text-muted-foreground mt-1">
                  {ticket.events?.event_end_at
                    ? new Date(ticket.events.event_end_at).getFullYear()
                    : ""}
                </span>
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    <Badge
                      ...
                      className="text-xs"
                      style={{
                        borderColor: getComputedColor(
                          ticket.ticket_types?.color,
                        ),
