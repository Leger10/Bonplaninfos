          ...
          <CardContent className="p-6">
            {!showRankings ? (
                <div className="flex flex-col items-center justify-center space-y-8 py-10">
                    <div className="w-64 h-32 bg-black/50 rounded-xl border-2 border-yellow-500/50 flex items-center justify-center shadow-[0_0_30px_rgba(234,179,8,0.3)]">
                        <span className="text-6xl font-mono font-bold text-yellow-400 tabular-nums">
                            {currentNumber}
                        </span>
                    </div>
...
const RaffleDrawSystem = ({ raffleData, eventId, isOrganizer, onDrawComplete }) => {
    ...
    const handleLaunchDraw = async () => {
        ...
        try {
            ...
            if (data.success) {
                toast({ title: "Tirage lancé !", description: "Les résultats sont en cours de génération." });
