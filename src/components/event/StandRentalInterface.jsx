...
import {
  Loader2, Store, CheckCircle, Info, DollarSign, Layout, Users, MapPin, Lock, ArrowRight,
  User, Phone, Building, BarChart3, PieChart, TrendingUp, Package, Grid3x3, Calendar, Clock,
  Wallet, Coins, Percent, AlertCircle, Sparkles, Zap, Star, Award, Rocket, Flame,
  Download, Copy, Mail, Map, Navigation, Printer, BadgeCheck, Share2, Eye, X,
  Briefcase, Tag, Hash, CalendarDays, UserCircle, FileText, CheckSquare, CreditCard,
  Ticket, Award as AwardIcon, Star as StarIcon, FileDown, Filter, Search, ChevronDown
} from 'lucide-react';
...
const StandRentalInterface = ({ event, isUnlocked, onRefresh, isClosed, userProfile }) => {
  ...
  useEffect(() => {
    // Filtrer les réservations en fonction de la recherche et du type sélectionné
    let filtered = [...allRentals];
    
    if (searchTerm) {
      filtered = filtered.filter(rental => 
        rental.company_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        rental.contact_person?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        rental.contact_email?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        rental.stand_number?.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
    
    if (selectedType !== 'all') {
      filtered = filtered.filter(rental => rental.stand_type_id === selectedType);
    }
  ...
  const calculateStatistics = () => {
    const totalStands = standTypes.reduce((sum, type) => sum + type.quantity_available, 0);
    const totalRentals = allRentals.length;
    const totalRevenue = allRentals.reduce((sum, rental) => sum + rental.rental_amount_pi, 0);
    ...
    const occupancyRate = totalStands > 0 ? (totalRentals / totalStands) * 100 : 0;
    ...
    const byType = standTypes.map(type => {
      ...
      const revenue = typeRentals.reduce((sum, r) => sum + r.rental_amount_pi, 0);
    ...
    try {
      ...
      if (user) {
        const { data: rentals, error: rentalError } = await supabase
          .from('stand_rentals')
          .select('*, stand_types(name, size, amenities, base_price, base_currency, calculated_price_pi, description), profiles(full_name, phone)')
          .eq('user_id', user.id)
          .eq('stand_event_id', standEvent.id)
          .maybeSingle();
  ...
  const handleRentStand = async () => {
    ...
    try {
      ...
      toast({
        title: "Erreur",
        description: error.message || "Impossible de réserver le stand",
        variant: "destructive"
      });
    } finally {
      setIsRenting(false);
  ...
  const exportToPDF = () => {
    ...
    try {
      ...
      doc.text(`Réservations de stands - ${event?.title || 'Événement'}`, 14, 20);
      ...
      doc.text(`Revenu total: ${statistics.totalRevenue} π (${statistics.totalRevenueFcfa.toLocaleString()} FCFA)`, 14, 55);
      doc.text(`Taux d'occupation: ${statistics.occupancyRate.toFixed(1)}%`, 14, 62);
      ...
      autoTable(doc, { // ✅ Utilisation correcte de autoTable
        head: [tableColumn],
        body: tableRows,
        startY: 70,
        theme: 'striped',
      ...
      // Ajouter un résumé par type de stand à la fin
      const finalY = doc.lastAutoTable.finalY + 10;
      
      doc.setFontSize(12);
      doc.text('Résumé par type de stand', 14, finalY);
      
      const summaryColumn = ['Type', 'Total', 'Réservés', 'Disponibles', 'Revenu (π)'];
      ...
      // Sauvegarder le PDF
      doc.save(`reservations-stands-${event?.title?.replace(/\s+/g, '-').toLowerCase() || 'evenement'}.pdf`);
  ...
  const exportToCSV = () => {
    try {
      ...
      link.setAttribute('download', `reservations-stands-${event?.title?.replace(/\s+/g, '-').toLowerCase() || 'evenement'}.csv`);
  ...
  if (myRental) {
    const colors = STAND_COLORS[standTypes.findIndex(t => t.id === myRental.stand_type_id) % STAND_COLORS.length];
      ...
      <div className="space-y-6">
        ...
        >
          <Card className="bg-gray-900 border-2 border-gray-800 shadow-2xl overflow-hidden relative">
            ...
            <div className={`absolute top-0 right-0 w-64 h-64 bg-gradient-to-br ${colors.gradient} rounded-full blur-3xl -mr-20 -mt-20 pointer-events-none`} />
            
            {/* Bandeau supérieur avec effet de ticket */}
            <div className={`relative bg-gradient-to-r ${colors.bg} border-b ${colors.border} p-6`}>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                      ...
                      <span className={`text-sm font-normal px-2 py-0.5 rounded-full ${colors.badge}`}>
                        {myRental.stand_types?.name}
                      </span>
                    </h2>
                    <p className="text-gray-400 text-sm mt-1">
                      {event?.title} • {new Date(event?.event_start_at).toLocaleDateString('fr-FR')}
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-base font-medium text-white">{myRental.stand_types?.name}</p>
                          <p className="text-sm text-gray-400">{myRental.stand_types?.size}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-xs text-gray-500">Numéro de stand</p>
                          <p className="text-2xl font-bold text-white">{myRental.stand_number}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xl font-bold text-white">{myRental.rental_amount_pi} π</p>
                          <p className="text-sm text-gray-400">
                            ≈ {(myRental.rental_amount_pi * COIN_RATE).toLocaleString()} FCFA
                          </p>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-base font-medium text-white">
                            {new Date(myRental.created_at).toLocaleDateString('fr-FR', {
                              day: '2-digit',
                              month: 'long',
                              year: 'numeric'
                            })}
                          ...
                          <p className="text-sm text-gray-400">
                            {new Date(myRental.created_at).toLocaleTimeString('fr-FR', {
                              hour: '2-digit',
                              minute: '2-digit'
                            })}
                          </p>
                  ...
                  <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
                    ...
                    <p className="text-gray-300 text-sm leading-relaxed">
                      {myRental.stand_types?.description || "Aucune description disponible"}
                    </p>
                  </div>

                  {myRental.stand_types?.amenities && Object.keys(myRental.stand_types.amenities).length > 0 && (
                    <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
                      <h3 className="text-white font-medium mb-3 flex items-center gap-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {Object.entries(myRental.stand_types.amenities)
                          .filter(([, v]) => v)
                          .map(([k]) => (
                            <Badge key={k} variant="outline" className="border-gray-600 text-gray-300 bg-gray-700/50">
                              {k.replace(/_/g, ' ')}
                            </Badge>
                          ))}
                      </div>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center justify-between">
                            <p className="text-white font-medium">{myRental.company_name}</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => copyToClipboard(myRental.company_name, "Nom de l'entreprise")}
                    ...
                    <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <p className="text-xs text-gray-500 mb-1">Personne à contacter</p>
                          <div className="flex items-center justify-between">
                            <p className="text-white font-medium">{myRental.contact_person}</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => copyToClipboard(myRental.contact_person, "Nom du contact")}
                              className="text-gray-400 hover:text-white"
                            >
                              <Copy className="w-3 h-3" />
                            </Button>
                    ...
                    <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center justify-between">
                            <p className="text-white font-medium">{myRental.contact_email}</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => copyToClipboard(myRental.contact_email, "Email")}
                              className="text-gray-400 hover:text-white"
                            >
                              <Copy className="w-3 h-3" />
                            </Button>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center justify-between">
                            <p className="text-white font-medium">
                              {myRental.contact_phone || myRental.profiles?.phone || 'Non renseigné'}
                            </p>
                            {myRental.contact_phone && (
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => copyToClipboard(myRental.contact_phone, "Téléphone")}
                                className="text-gray-400 hover:text-white"
                              >
                                <Copy className="w-3 h-3" />
                              </Button>
                    ...
                    {myRental.business_description && (
                      <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-xs text-gray-500 mb-1">Activité / Produits exposés</p>
                            <p className="text-gray-300 text-sm">{myRental.business_description}</p>
                          </div>
                        </div>
                      </div>
                    )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-start gap-3 bg-gray-800/30 rounded-lg p-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-400">
                        {new Date(event?.event_start_at).toLocaleTimeString('fr-FR', { 
                          hour: '2-digit', 
                          minute: '2-digit' 
                        })}
                      </p>
              ...
              <div className="mt-6 p-4 bg-gray-800/50 rounded-xl border border-gray-700">
                ...
                <ul className="space-y-2">
                  <li className="flex items-start gap-2 text-sm text-gray-300">
                    ...
                    <span>Présentez-vous à l'accueil de l'événement avec cette confirmation.</span>
                  </li>
                  <li className="flex items-start gap-2 text-sm text-gray-300">
                    <CheckCircle className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />
                    <span>Votre stand <span className="text-white font-medium">{myRental.stand_number}</span> vous sera attribué à votre arrivée.</span>
                  </li>
                  <li className="flex items-start gap-2 text-sm text-gray-300">
                    <CheckCircle className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />
                    <span>Un email de confirmation a été envoyé à <span className="text-white">{myRental.contact_email}</span>.</span>
                  </li>
                </ul>
              </div>
        ...
        {/* Liste des réservations pour l'organisateur */}
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                  ...
                  <CardDescription className="text-gray-400">
                    {allRentals.length} entreprise{allRentals.length > 1 ? 's' : ''} ont réservé un stand
                  </CardDescription>
                </div>
              ...
              {filteredRentals.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {filteredRentals.map((rental) => {
                        ...
                        const colors = STAND_COLORS[typeIndex % STAND_COLORS.length];

                        return (
                          <TableRow key={rental.id} className="border-gray-800 hover:bg-gray-800/50">
                            <TableCell className={`font-medium ${colors.text}`}>{rental.stand_number}</TableCell>
                            <TableCell>
                              <div className="text-white">{rental.company_name}</div>
                            </TableCell>
                            <TableCell>
                              <div className="text-gray-300">{rental.contact_person}</div>
                            </TableCell>
                            <TableCell>
                              <div className="text-gray-300 text-sm">{rental.contact_email}</div>
                            </TableCell>
                            <TableCell>
                              <div className="text-gray-400 text-sm">{rental.contact_phone || rental.profiles?.phone || 'N/A'}</div>
                            </TableCell>
                            <TableCell>
                            ...
                            <TableCell className="text-right">
                              <span className="text-white font-medium">{rental.rental_amount_pi} π</span>
                              <span className="text-gray-400 text-sm ml-2">
                                ({(rental.rental_amount_pi * COIN_RATE).toLocaleString()} F)
                              </span>
                            </TableCell>
                            <TableCell className="text-center">
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  copyToClipboard(contactInfo, "Informations de contact");
                                }}
                                className="text-gray-400 hover:text-white"
                              >
                  ...
                  <div className="mt-4 text-sm text-gray-400">
                    Affichage de {filteredRentals.length} réservation{filteredRentals.length > 1 ? 's' : ''} sur {allRentals.length} au total
                  </div>
                </div>
              )}
            </CardContent>
  ...
  return (
    <div className="space-y-6">
      ...
      {userProfile?.user_type === 'organizer' && user?.id === event?.organizer_id && standTypes.length > 0 && (
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
              <div className="bg-gray-800/50 p-4 rounded-lg border border-gray-700">
                ...
                <p className="text-2xl font-bold text-white">{statistics.totalRentals}</p>
                <p className="text-xs text-gray-500 mt-1">sur {standTypes.reduce((sum, t) => sum + t.quantity_available, 0)} disponibles</p>
              </div>

              <div className="bg-gray-800/50 p-4 rounded-lg border border-gray-700">
                <p className="text-sm text-gray-400">Revenus bruts</p>
                <p className="text-2xl font-bold text-white">{statistics.totalRevenue} π</p>
                <p className="text-xs text-gray-500 mt-1">{statistics.totalRevenueFcfa.toLocaleString()} FCFA</p>
              </div>

              <div className="bg-gray-800/50 p-4 rounded-lg border border-gray-700">
                <p className="text-sm text-gray-400">Frais plateforme</p>
                <p className="text-2xl font-bold text-orange-400">-{statistics.totalCommission.toFixed(0)} F</p>
                <p className="text-xs text-gray-500 mt-1">5% commission</p>
              </div>

              <div className="bg-gray-800/50 p-4 rounded-lg border border-gray-700">
                <p className="text-sm text-gray-400">Vos gains nets</p>
                <p className="text-2xl font-bold text-green-400">{statistics.totalNetGain.toFixed(0)} F</p>
                <p className="text-xs text-gray-500 mt-1">après commission</p>
              </div>

              <div className="bg-gray-800/50 p-4 rounded-lg border border-gray-700">
                <p className="text-sm text-gray-400">Taux d'occupation</p>
                <p className="text-2xl font-bold text-white">{statistics.occupancyRate.toFixed(1)}%</p>
                <Progress value={statistics.occupancyRate} className="h-1.5 mt-2 bg-gray-700" />
              </div>
            </div>

            {/* Détail par type de stand */}
            <div className="mt-6">
              <h4 className="text-white font-medium mb-3">Détail par type de stand</h4>
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow className="border-gray-800">
                      <TableHead className="text-gray-400">Type</TableHead>
                      <TableHead className="text-gray-400 text-center">Dimensions</TableHead>
                      <TableHead className="text-gray-400 text-center">Prix</TableHead>
                      <TableHead className="text-gray-400 text-center">Disponibles</TableHead>
                      <TableHead className="text-gray-400 text-center">Réservés</TableHead>
                      <TableHead className="text-gray-400 text-center">Taux</TableHead>
                      <TableHead className="text-gray-400 text-right">Revenus</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {statistics.byType.map((type, index) => {
                      ...
                      return (
                        <TableRow key={type.id} className="border-gray-800 hover:bg-gray-800/50">
                          <TableCell>
                          ...
                          <TableCell className="text-center text-gray-300">{type.size}</TableCell>
                          <TableCell className="text-center text-white font-medium">{type.calculated_price_pi} π</TableCell>
                          <TableCell className="text-center text-gray-400">{type.quantity_available}</TableCell>
                          <TableCell className="text-center">
                          ...
                          <TableCell className="text-center">
                            <div className="flex items-center gap-2">
                              <span className="text-white">{type.rate.toFixed(1)}%</span>
                              <Progress value={type.rate} className="w-16 h-1.5 bg-gray-700" />
                            </div>
                          </TableCell>
                          <TableCell className="text-right text-white font-medium">{type.revenue} π</TableCell>
                        </TableRow>
                      );
                    })}
                  </TableBody>
          ...
          {standTypes.map((type, index) => {
            ...
            return (
              <motion.div
                ...
                className="h-full"
              >
                <Card className={`bg-gray-900 border-l-4 ${colors.border} border-gray-800 h-full flex flex-col hover:shadow-xl hover:shadow-${colors.text}/10 transition-all`}>
                  <CardHeader className="border-b border-gray-800 pb-4">
                    <div className="flex justify-between items-start">
                      ...
                      <Badge
                        ...
                        className={isClosed ? "border-gray-600 text-gray-400" : isSoldOut ? "bg-red-900 text-red-200" : colors.badge}
                      >
                  ...
                  <CardContent className="flex-1 pt-4">
                    ...
                    {type.amenities && Object.keys(type.amenities).length > 0 && (
                      <div className="flex flex-wrap gap-1 mb-4">
                        {Object.entries(type.amenities)
                          .filter(([, v]) => v)
                          .map(([k]) => (
                            <Badge key={k} variant="outline" className="border-gray-700 text-gray-300 bg-gray-800/50">
                              {k.replace(/_/g, ' ')}
                            </Badge>
                          ))}
                      </div>
                    ...
                    {/* Statistiques pour l'organisateur */}
                    {userProfile?.user_type === 'organizer' && user?.id === event?.organizer_id && (
                      <div className="mt-2 p-3 bg-gray-800/50 rounded-lg border border-gray-700">
                        <div className="flex justify-between text-sm mb-1">
                          ...
                          <span className={`font-medium ${colors.text}`}>
                            {type.quantity_rented || 0}/{type.quantity_available}
                          </span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-400">Revenu:</span>
                          <span className="text-white font-medium">
                            {(type.quantity_rented || 0) * type.calculated_price_pi} π
                          </span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between w-full">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          ≈ {(type.calculated_price_pi * COIN_RATE).toLocaleString()} F
                        </p>
                      </div>

                      {isClosed ? (
                        <Button disabled variant="outline" size="sm" className="border-gray-700 text-gray-500">
                          Terminé
                        </Button>
                      ) : isSoldOut ? (
                        <Button disabled variant="outline" size="sm" className="border-gray-700 text-gray-500">
                          ...
                          <DialogContent className="bg-gray-900 border-gray-800 text-white">
                            ...
                            <div className="grid gap-4 py-4">
                              <div className="grid gap-2">
                                <Label htmlFor="company" className="text-gray-300">Nom de l'entreprise / Marque *</Label>
                                <Input
                                  id="company"
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="grid gap-2">
                                  <Label htmlFor="contact" className="text-gray-300">Personne à contacter *</Label>
                                  <Input
                                    id="contact"
                              ...
                              <div className="grid gap-2">
                                <Label htmlFor="desc" className="text-gray-300">Activité / Produits exposés</Label>
                                <Textarea
                                  id="desc"
