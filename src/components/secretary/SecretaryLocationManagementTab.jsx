...
const SecretaryLocationManagementTab = ({ onRefresh }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // Corrected query to explicitly join profiles on user_id
      const { data, error } = await supabase
        .from('locations')
        .select('*, profiles!locations_user_id_fkey(full_name, email)')
  ...
  const filteredLocations = useMemo(() => {
    ...
    return locations.filter(loc =>
      ...
      (loc.profiles?.full_name && loc.profiles.full_name.toLowerCase().includes(lowerSearch))
    );
            ...
            <div key={loc.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-background/50 rounded-lg gap-4">
              <div className="flex items-center gap-4 flex-grow">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Ajouté par: {loc.profiles?.full_name || 'N/A'}</p>
