...
const CreatePartnerForm = ({
  ...
  const filteredUsers = useMemo(() => {
    if (!userSearch)
      return users.filter(
        (u) => u.user_type !== "admin" && u.user_type !== "super_admin",
      );
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.partner_id || !formData.country || !formData.license_id) {
      toast({
        ...
        description:
          "Veuillez sélectionner un utilisateur, un pays et un type de licence.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (formData.cities.length > 1) {
      toast({
        ...
        description:
          "Un administrateur ne peut couvrir qu’une seule ville ou tout le pays. Sélectionnez au plus une ville.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const selectedCity =
        formData.cities.length === 1 ? formData.cities[0] : null;
      ...
      if (partnerToEdit) {
        ...
        if (partnerToEdit.status === "active") {
          ...
          if (licenseData?.duration_days) {
            ...
            expiration_date.setDate(
              expiration_date.getDate() + licenseData.duration_days,
            );
        ...
        toast({
          ...
          description: "Le nouveau partenaire est en attente d'approbation.",
        });
      }
        ...
        <CardContent>
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <Select
                onValueChange={(v) =>
                  setFormData({ ...formData, license_id: v })
                }
                value={formData.license_id}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Sélectionner un type de licence" />
                </SelectTrigger>
                <SelectContent>
                  {licenseTypes.map((lt) => (
            ...
            <div className="space-y-2">
              ...
              <Select
                onValueChange={(v) =>
                  setFormData({ ...formData, country: v, cities: [] })
                }
                value={formData.country}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Pays de la licence" />
                </SelectTrigger>
                <SelectContent>
                  {COUNTRIES.map((c) => (
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Ne sélectionnez aucune ville pour couvrir tout le pays.
              </p>
            </div>
...
const PartnersManagement = ({ onRefresh }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data: partnersData, error: partnersError } = await supabase
        .from("partners")
        .select(
          "*, user:user_id(full_name, email), license:license_id(name, commission_rate)",
        );
  ...
  const handleUpdateStatus = async (partnerId, newStatus) => {
    try {
      ...
      if (newStatus === "active") {
        ...
        const selectedCity =
          partner.coverage_zone.cities?.length === 1
            ? partner.coverage_zone.cities[0]
            : null;
  ...
  const handleDeletePartner = async (partnerId) => {
    ...
    try {
      ...
      toast({ title: "Partenaire supprimé avec succès" });
  ...
  const getStatusForPartner = (partner) => {
    const now = new Date();
    const expirationDate = partner.expiration_date
      ...
      <Card className="glass-effect shadow-lg rounded-xl border-primary/20">
        <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          <Button onClick={() => setShowCreateForm(true)}>
            <UserPlus className="mr-2 h-4 w-4" /> Créer un partenaire
          </Button>
        </CardHeader>
        <CardContent>
          <div className="mb-4">
                      ...
                      <td className="px-6 py-4">
                        <p className="text-white">
                          {partner.expiration_date
                            ? `Expire le: ${new Date(partner.expiration_date).toLocaleDateString()}`
                            : "N/A"}
                        </p>
                      </td>
                      <td className="px-6 py-4">
