  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      let query = supabase
        ...
        `)
        .in('action_type', ['withdrawal_approved', 'withdrawal_rejected'])
                  ...
                  <TableRow key={log.id}>
                    ...
                    <TableCell>
                       <span className={`px-2 py-1 rounded text-xs ${
                        log.action_type === 'withdrawal_approved' ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"
                       }`}>
                         {log.action_type === 'withdrawal_approved' ? 'Approuvé' : 'Rejeté'}
                       </span>
                    </TableCell>
                    {!actorId && (
                      <TableCell>
                    ...
                    <TableCell className="text-xs">{log.details?.rejection_reason || '—'}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
